...
use app\modules\wedatabase\forms\WEDatabaseBackUpForm;

class WEDatabaseBackUpAction extends Action {
    public function run() {
        $form = new WEDatabaseBackUpForm;
        $form->setAttributes(Yii::$app->request->get());
        if(Yii::$app->request->isGet) {
            $form->setAttributes(Yii::$app->request->get());
            if($form->dbname) {
                return $this->controller->render("backup", array(
                    "dbname" => $form->dbname,
                ));
            } else {
            ...
            if($form->validate()) {
                ...
                $builder->append(WEParamsUtil::get("serviceDbDumpApi"));
